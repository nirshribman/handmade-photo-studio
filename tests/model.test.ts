...
describe('Continuous, deterministic layout',()=>{
 it('all partitions cover a numbered checkerboard once',()=>{for(const mode of ['single','strips','grid']){const p=neutral();p.layout.mode=mode;const g=buildGeometry(p,600,400);expect(g.pieces.reduce((s,p)=>s+p.width*p.height,0)).toBeCloseTo(g.width*g.height,5);for(let y=5;y<g.height;y+=11)for(let x=5;x<g.width;x+=13)expect(g.pieces.filter(p=>x>=p.x&&x<p.x+p.width&&y>=p.y&&y<p.y+p.height)).toHaveLength(1);}});
 it('gaps translate pieces without changing source partitions or tear paths',()=>{const p=presets[2],before=buildGeometry(p,600,400);const q=structuredClone(p);q.layout.gapPct=6;const after=buildGeometry(q,600,400);before.pieces.forEach((v,i)=>{expect(v.points).toEqual(after.pieces[i].points);expect([v.x,v.y,v.width,v.height]).toEqual([after.pieces[i].x,after.pieces[i].y,after.pieces[i].width,after.pieces[i].height]);});expect(after.pieces[1].dy).not.toBe(before.pieces[1].dy);});
 it('unrelated controls do not change material/contour seeds or geometry',()=>{const p=initialProject(),g=buildGeometry(p,600,400),sample=noise(12.23,45.67,p.paper.seed);p.tone.exposureEv=1.2;p.lighting.azimuthDeg=100;expect(buildGeometry(p,600,400)).toEqual(g);expect(noise(12.23,45.67,p.paper.seed)).toBe(sample);});
 it('grid variation has weighted zero mean and independent placement',()=>{const p=presets[3],g=buildGeometry(p,600,400);expect(g.pieces.reduce((s,v)=>s+v.tone*v.width*v.height,0)).toBeCloseTo(0,6);const q=structuredClone(p);q.layout.toneVariation=100;expect(buildGeometry(q,600,400).pieces.map(x=>[x.dx,x.dy,x.angle])).toEqual(g.pieces.map(x=>[x.dx,x.dy,x.angle]));});
 it('rejects unordered, inconsistent and out-of-bounds crops',()=>{const p=neutral();p.layout.cutPositions=[.8,.2];expect(()=>validateProject(p)).toThrow('ordered');p.layout.cutPositions=[.3,.7];p.composition.crop.x=.4;expect(()=>validateProject(p)).toThrow('Crop');expect(partitionAxes(neutral(),1500,1000)).toEqual({xs:[0,1500],ys:[0,1000]});});
});
...
// Edge regressions: old recipes gain neutral controls; adjacent torn pieces must match.
describe('Fibrous edge recipes',()=>{
 ...
 it('identifies shared cuts exactly and keeps framing fixed as fibres change',()=>{
  const p=initialProject();p.layout.mode='grid';p.layout.rows=2;p.layout.columns=2;p.layout.tearAmount=100;p.edges.strength=100;p.edges.irregularity=100;
  const g=buildGeometry(p,600,400),[a,b,c]=g.pieces;
