...
test('choose three photos, individual crop and tilt, undo, saved restore and real frame export',async({page})=>{
 ...
 await page.getByRole('button',{name:'Project options'}).click();await page.getByRole('button',{name:'Save project locally',exact:true}).click();await expect(page.getByRole('status')).toContainText('including all original photos');
