...
## Album backgrounds and rounded corners - renderer 0.6.0

Schema 6 adds five original procedural album materials, plain colour, independent texture/age/foxing/edge-aging/binding controls, texture scale and seed, and black/ivory/kraft mounting pockets. Corner rounding can affect only the outside sheet or every piece; uneven radii and narrow worn-rim shading share the canonical geometry. The pockets follow piece rotation. Existing schemas 1-5 migrate, preserving plain backgrounds and square corners for old recipes.

Validation: all 23 unit tests and 15 browser scenarios passed; TypeScript/Vite production build passed. The album tests cover every background material and scalar control, deterministic seeds, unchanged photo interiors, rounded alpha masks, transparent and image-only output, preview/export agreement, and worker/main-thread equality. UI checks cover one-step preset undo/redo, live canvas feedback, local save/restore after reload, settings JSON, real downloaded 1,400-pixel PNG and JPEG, and a 390-pixel mobile layout. No runtime errors occurred in the final UI run. The development server was restarted after a stale cached JSON control registry was detected during testing.

The album pixel report records zero difference in photo interiors, transparent output, image-only output and worker/main-thread results. Repeated rendering is byte-identical. A 1,800 x 1,350 export downsampled against a 600 x 450 preview has mean absolute RGBA error 0.846/255 (maximum 63 at resampled detail); this does not claim identical pixels across different resolutions. Background fields remain active at Finish strength zero, as intentional arrangement settings.

Inspected the supplied online references, five 2,000-pixel album exports, the 2,200-pixel rounded full-bleed print, a 2,400-pixel tilted three-photo sequence, transparent output comparisons, corner details, actual UI-downloaded JPEG and desktop/mobile screenshots. Corrected excessive domain warping that initially resembled wood grain, warmed the aging absorption colour, and adjusted cloth weave spacing and antialiasing for export visibility. No stock/reference imagery is embedded. These are procedural material interpretations; captured-library and physical-fidelity limitations above remain.

Exports and recipes are in artifacts/albums; scripts/album-gallery.mjs builds the local comparison gallery. The final production main chunk is approximately 519 KB minified; Vite reports its existing 500 KB advisory. Personal photographs, reference images and generated review exports are excluded from the repository.
