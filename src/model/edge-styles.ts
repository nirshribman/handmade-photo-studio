...
export const cornerDefaults={"cornerRadius":0,"cornerVariation":35,"cornerScope":"sheet","rimWear":0};
export const edgeDefaults={outerFray:100,innerFray:100,exposedPaper:0,fibreLength:25,longFibres:0,fibreClumping:45};
export const edgeStyles=[
 {id:'rounded',name:'Rounded washi',note:'Soft uneven corners with a gently worn, fibrous rim',values:{profile:'deckled',strength:100,irregularity:45,roughness:48,featherWidth:20,looseFibres:26,exposedPaper:14,fibreLength:20,longFibres:4,fibreClumping:65,cornerRadius:4.5,cornerVariation:60,cornerScope:'sheet',rimWear:65}},
...
export function applyEdgeStyle(p:Project,id:string):Project {const style=edgeStyles.find(s=>s.id===id);if(!style)throw new Error('Unknown edge style.');return {...p,edges:{...p.edges,...cornerDefaults,...style.values,enabled:true},layout:p.layout.mode==='single'?p.layout:{...p.layout,tearAmount:Math.max(60,p.layout.tearAmount)}};}
