...
export class Renderer {
 ...
 constructor(){
  ...
  this.capability={webgl2:true,maxTexture:gl.getParameter(gl.MAX_TEXTURE_SIZE),maxRenderbuffer:gl.getParameter(gl.MAX_RENDERBUFFER_SIZE),maxViewport:Array.from(gl.getParameter(gl.MAX_VIEWPORT_DIMS)),maxPixels:0,maxSide:0,gpu:debug?gl.getParameter(debug.UNMASKED_RENDERER_WEBGL):gl.getParameter(gl.RENDERER),adapter:'WebGL2'};
  this.capability.maxSide=Math.min(this.capability.maxTexture,this.capability.maxRenderbuffer,...this.capability.maxViewport);
  this.capability.maxPixels=this.capability.maxSide*this.capability.maxSide;
 ...
 private target(name:string,w:number,h:number){
  const old=this.targets.get(name);if(old&&old.width===w&&old.height===h)return old;
  const gl=this.gl;if(old){gl.deleteTexture(old.texture);gl.deleteFramebuffer(old.framebuffer);this.targets.delete(name);}
  const texture=this.newTexture();let framebuffer:WebGLFramebuffer|null=null;
  try{gl.bindTexture(gl.TEXTURE_2D,texture);gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA8,w,h,0,gl.RGBA,gl.UNSIGNED_BYTE,null);framebuffer=gl.createFramebuffer();if(!framebuffer)throw new Error('Framebuffer allocation failed.');gl.bindFramebuffer(gl.FRAMEBUFFER,framebuffer);gl.framebufferTexture2D(gl.FRAMEBUFFER,gl.COLOR_ATTACHMENT0,gl.TEXTURE_2D,texture,0);if(gl.getError()!==gl.NO_ERROR||gl.checkFramebufferStatus(gl.FRAMEBUFFER)!==gl.FRAMEBUFFER_COMPLETE)throw new Error('Texture allocation failed.');const t={texture,framebuffer,width:w,height:h};this.targets.set(name,t);return t;}
  catch{gl.deleteTexture(texture);if(framebuffer)gl.deleteFramebuffer(framebuffer);throw new Error('Not enough graphics memory for '+w+' x '+h+' pixels. Try a smaller output or close other graphics-heavy tabs.');}
 }
 private resizeDrawingBuffer(w:number,h:number){this.canvas.width=w;this.canvas.height=h;if(this.gl.drawingBufferWidth!==w||this.gl.drawingBufferHeight!==h||this.gl.isContextLost())throw new Error('The graphics device could not allocate '+w+' x '+h+' pixels. Try a smaller output.');}
 ...
 // The default WebGL drawing buffer can be smaller than MAX_TEXTURE_SIZE.
 // Keep large results in textures, then copy bounded blocks into the 2D surface.
 // Each block uses a blit and canvas copy so premultiplied alpha is preserved.
 private largeSurface(w:number,h:number){return w>4096||h>4096||w*h>8000000;}
 private copyTarget(target:Target,dest:Surface){
  const gl=this.gl,w=target.width,h=target.height;
  if(!this.largeSurface(w,h)){this.resizeDrawingBuffer(w,h);gl.bindFramebuffer(gl.READ_FRAMEBUFFER,target.framebuffer);gl.bindFramebuffer(gl.DRAW_FRAMEBUFFER,null);gl.blitFramebuffer(0,0,w,h,0,0,w,h,gl.COLOR_BUFFER_BIT,gl.NEAREST);this.copyGL(dest);return;}
  const block=2048,bw=Math.min(block,w),bh=Math.min(block,h);this.resizeDrawingBuffer(bw,bh);dest.width=w;dest.height=h;const ctx=context2d(dest);ctx.clearRect(0,0,w,h);
  for(let y=0;y<h;y+=block)for(let x=0;x<w;x+=block){const tw=Math.min(block,w-x),th=Math.min(block,h-y);gl.bindFramebuffer(gl.READ_FRAMEBUFFER,target.framebuffer);gl.bindFramebuffer(gl.DRAW_FRAMEBUFFER,null);gl.blitFramebuffer(x,y,x+tw,y+th,0,0,tw,th,gl.COLOR_BUFFER_BIT,gl.NEAREST);ctx.save();ctx.translate(x,y+th);ctx.scale(1,-1);ctx.drawImage(this.canvas,0,bh-th,tw,th,0,0,tw,th);ctx.restore();}
 }
 ...
 render(p:Project,o:RenderOptions):{canvas:Surface;metrics:RenderMetrics}{
  ...
  if(!Number.isInteger(o.width)||!Number.isInteger(o.height)||Math.min(o.width,o.height)<1||Math.max(o.width,o.height)>this.capability.maxSide||o.width*o.height>this.capability.maxPixels)throw new Error(`Output exceeds this device’s limit (${this.capability.maxSide}px per side).`);
  ...
  if(o.includeBackground){
   if(albumVisible&&p.stage.material!=='plain'){
    ...
    if(key!==this.albumKey){const target=this.largeSurface(o.width,o.height)?this.target('album',o.width,o.height):null;if(!target)this.resizeDrawingBuffer(o.width,o.height);const program=this.pass('album',target,o.width,o.height);this.uniform(program,'stageSize',[o.width/scale,o.height/scale]);this.uniform(program,'baseColour',rgb(o.flattenColour??p.stage.colour));this.uniform(program,'paperSettings',[p.stage.texture/100,p.stage.aging/100,p.stage.stains/100,p.stage.edgeShade/100]);this.uniform(program,'pageSettings',[p.stage.binding/100,0,p.stage.textureScale]);this.uniform(program,'material',backgroundStyles.findIndex(s=>s.id===p.stage.material),true);gl.uniform1ui(gl.getUniformLocation(program,'albumSeed'),p.stage.seed);this.draw();if(target)this.copyTarget(target,this.album);else this.copyGL(this.album);this.albumKey=key;}
  ...
  if(imageView){
   ...
   this.copyTarget(photoTarget,this.face);out.drawImage(this.face,0,0,o.width,o.height);
  }else{
   const faceTarget=this.largeSurface(fw,fh)?this.target('material',fw,fh):null;if(!faceTarget)this.resizeDrawingBuffer(fw,fh);const pr=this.pass('material',faceTarget,fw,fh);this.texture(pr,'photo',photoTarget.texture,0);
